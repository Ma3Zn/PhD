...
# Function that generate the SparsePauiliOp representation of the excitation 
# operators to be used in the subspace expansion
def build_exitation_operators(n_spin: int, loc: bool = False) \
    -> tuple[list[SparsePauliOp], int]:
    return crea_operatori_eccitazione_SbE(n_spin)
...
def energy_estimate_SbE(circ: QuantumCircuit,       \
                        ...
                        n_op: int)                  \
                        -> float:
    return stima_energia_SbE(circ, H, op, POVM, max_pow, n_op)