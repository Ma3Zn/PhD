...
spin_vals = [5/2]; % Da individuare l'intervallo opportuno anche per il 7/2

% Numero campi da misurare
n_Bx = 20;

% Valori del campo da misurare
Bx_vals(1,:) = [logspace(-5,-5.95,n_Bx/2) logspace(-6.20, -6.80, 3/2*n_Bx)];
Bx_vals(2,:) = [logspace(-5,-6.70,n_Bx/2) logspace(-6.70, -7.60, 3/2*n_Bx)];
Bx_vals(3,:) = [logspace(-5,-7.65,n_Bx/2) logspace(-7.65, -8.45, 3/2*n_Bx)];
...
fai_plot(T_mis * 1e-9, Bx_vals, spin_vals(1:n_spin), false, {'$\tau$ [ s ]', '$B_{x}$ [ T ]', '_Tm_','_Bxe_',num2str(T_rot_vals(1)),num2str(lv_pop),num2str(T_rot_vals(1))});
