...
Bx_vals(1,:) = [logspace(-5,-5.85,n_Bx/2) logspace(-5.85, -6.05, 3/2*n_Bx)];
Bx_vals(2,:) = [logspace(-5,-6.6,n_Bx/2)  logspace(-6.60, -7.05, 3/2*n_Bx)];
Bx_vals(3,:) = [logspace(-5,-7.25,n_Bx/2) logspace(-7.25, -7.85, 3/2*n_Bx)];
...
%       Settiamo il valore di T_max
        if (j > 1)
            T_max = 2*(1+log10(Bx_old/Bx)) * T_mis(i,j);
            Bx_old = Bx;
        end
...
% % % fai_plot(T_es * 1e-9, Bx_vals, spin_vals(1:n_spin), false, {'$\Delta t$ [ s ]', '$B_{x}$ [ T ]', '_Tm_','_Bxe_',num2str(T_rot_vals(1)),num2str(lv_pop),num2str(T_rot_vals(1)*100)});
% % % 
% % % %%  Plot Sensitivity
% % % 
% % % fai_plot(T_es * 1e-9, sensitivity, spin_vals(1:n_spin), false, {'$\Delta t$ [ s ]', '$\eta$ [ THz$^{-1/2}$ ]', '_sensitivity','',num2str(T_rot_vals(1)),num2str(lv_pop),num2str(T_rot_vals(1)*100)});
% % % fai_plot(Bx_vals, derivata, spin_vals(1:n_spin), false, {'$B_{x}$', '|$\frac{\partial T}{\partial B_x}|^{-1}$', '_derivata','',num2str(T_rot_vals(1)),num2str(lv_pop),num2str(T_rot_vals(1)*100)});
