...
%           Cicliamo sugli stati iniziali del control
            for sup_in_c = init_superposition
%               Cicliamo sugli stati iniziali del target
                for sup_in_t = init_superposition
%                   Generiamo l'input per la simulazione (supponiamo la durata
%                   del gate fisico di 10 ns)
                    in = genera_input_1_2(T2, R_par, sup_in_c, sup_in_t, 2);
...
%       operazioni
        errore_massimo(idx_T2) = max(errore_op);
        errore_minimo(idx_T2)  = min(errore_op);
