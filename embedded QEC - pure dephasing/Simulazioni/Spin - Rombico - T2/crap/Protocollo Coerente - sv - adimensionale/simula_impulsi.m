...
%   Allochiamo lo spazio per l'evoluzione dello state vector e della
%   matrcie di densità logica
    sim.evol   = zeros(dim, Ntime+1);
    sim.evol_L = zeros(dim, dim, Ntime+1);

%   Scriviamo lo stato iniziale del sistema in base logica
...
% % %     sim.evol(:,1)   = sim.sys.psi_in;
% % %     sim.evol_L(:,:,1) = sim.sys.CB' * sim.evol(:,1) * sim.evol(:,1)' * sim.sys.CB;

    %   condizine iniziale del sistema
    y0 = sim.evol(:,1);
...
%   Reshape delle soluzioni approssimate
    for i = 1:Ntime
        sim.evol(:,i+1)   = y(i,:).';
        sim.evol_L(:,:,i+1) = sim.sys.CB' * sim.evol(:,i) * sim.evol(:,i)' * sim.sys.CB;
    end
