...
%%  Primo Impulso   
    ...
    imp_1.fase    = angle(h(lv(1),lv(2))) + pi/2;
...
%%  Secondo Impulso   
    ...
    imp_2.fase    = angle(h(lv(1),lv(2))) + pi/2;
%     imp_2.angolo  = pi/2; %%%%%%%%%%%%%%% DBG
%     imp_2.fase    = 0 + pi/2; %%%%%%%%%%%%%%% DBG
    
%   Rimozione completa leackage
    imp_2.Mz               = zeros(dim, dim)%   Settiamo in modo opportuno le ampiezze dei relativi campi
% % %     B_max = 0.2;
% % %     imp_1.Bz = B_max;
% % %     imp_2.Bz = B_max;
% % %     imp_3.Bz = B_max;
...
%%  Terzo Impulso   
    ...
    imp_3.fase    = angle(h(lv(1),lv(2))) + pi/2;
%     imp_3.angolo  = pi/2; %%%%%%%%%%%%%%% DBG
%     imp_3.fase    = 0 + pi/2; %%%%%%%%%%%%%%% DBG
...
%%  Quarto Impulso   
    ...
    imp_4.fase    = angle(h(lv(1),lv(2))) + pi/2;
%     imp_4.angolo  = pi/2; %%%%%%%%%%%%%%% DBG
%     imp_4.fase    = 0 + pi/2; %%%%%%%%%%%%%%% DBG
...
%%  Settaggio ampiezze campi oscillanti
    K_12 = abs(imp_1.angolo * imp_1.w / abs(sys.Mx(1,2) * imp_1.d_Sz) )
    K_13 = abs(imp_2.angolo * imp_2.w / abs(sys.Mx(1,3) * imp_2.d_Sz) )
    K_24 = abs(imp_3.angolo * imp_3.w / abs(sys.Mx(2,4) * imp_3.d_Sz) )
    K_34 = abs(imp_4.angolo * imp_4.w / abs(sys.Mx(3,4) * imp_4.d_Sz) )

% % % %   Per velocizzare le simulazioni
% % %     B_max = 1e1 * B_max;
