...
%   anche un errore di misura nelle simulazioni
    [prj_gen, p] = genera_proiettori_generalizzati(prj, dim, 1e-2);
...
%   delle operazioni di recovery (sul sistema ridotto)
    ...
    for k = 1 : dim/2
        prj_gen{k} = kron(CB, eye(dim/2)) * prj_gen{k} * kron(CB', eye(dim/2));
...
%   Riempiamo sim con i parametri necessari
    sim.gamma     = gamma;      % matrice per la costruzione dei superoperatori d'errore
    ...
    sim.prj       = prj;        % proiettori ancilla
    sim.p_prj_gen = p;          % distribuzione probabilità proiettori generalizzati
    sim.prj_gen   = prj_gen;    % proiettori ancilla generalizzati, i.e., "storti"
